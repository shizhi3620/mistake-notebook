...
export class InMemoryRecognitionTaskStore implements RecognitionTaskStore {
  ...
  async consumeHomeworkUploadCredential(parentAccountId: string, uploadToken: string, fileId: string, now = Date.now()) { const credential = this.uploads.get(uploadToken); if (!credential || credential.parentAccountId !== parentAccountId || credential.usedAt !== null || credential.expiresAt <= now || !fileId.endsWith(`/${credential.imageKey}`)) throw new Error("Homework upload credential is not valid."); credential.usedAt = now; return credential; }
}
...
export class MysqlRecognitionTaskStore implements RecognitionTaskStore {
  ...
  async consumeHomeworkUploadCredential(parentAccountId: string, uploadToken: string, fileId: string, now = Date.now()) { const [rows] = await this.pool.query<RowDataPacket[]>("SELECT * FROM homework_upload_credentials WHERE upload_token=? AND parent_account_id=?", [uploadToken,parentAccountId]); const credential = rows[0] ? uploadRow(rows[0]) : undefined; if (!credential || credential.usedAt !== null || credential.expiresAt <= now || !fileId.endsWith(`/${credential.imageKey}`)) throw new Error("Homework upload credential is not valid."); const [result] = await this.pool.execute("UPDATE homework_upload_credentials SET used_at=? WHERE upload_token=? AND used_at IS NULL", [now,uploadToken]); if ((result as { affectedRows: number }).affectedRows !== 1) throw new Error("Homework upload credential is already used."); return { ...credential, usedAt: now }; }
}
