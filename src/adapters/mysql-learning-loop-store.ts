...
export class MysqlLearningLoopStore {
  ...
  async saveReview(v:Review):Promise<void>{await this.pool.execute("UPDATE reviews SET completed_at=?,self_assessment=?,variant_correct=?,result_interval_index=?,result_next_review_at=?,result_mastery_score=? WHERE id=? AND parent_account_id=?",[v.completedAt,v.selfAssessment,v.variantCorrect,v.resultIntervalIndex,v.resultNextReviewAt,v.resultMasteryScore,v.id,v.parentAccountId]);}
  ...
  async findHomeworkReview(parentAccountId:string,id:string):Promise<HomeworkReview|undefined>{const r=await this.one<RowDataPacket & Record<string,unknown>>("SELECT * FROM homework_reviews WHERE id=? AND parent_account_id=?",[id,parentAccountId]);return r&&{id:String(r.id),parentAccountId:String(r.parent_account_id),childProfileId:String(r.child_profile_id),imageKey:r.image_key as string|null,createdAt:Number(r.created_at),candidates:JSON.parse(String(r.candidates_json))};}
